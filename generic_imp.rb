...
symbols.each { |s|
    ...
    if t_def[/\W#{template}\W*?[,\(\)]/][-1] != ')'
        ...
        sym_refs << "#define #{template}_Ref(#{t_arg_names.map(&:format).join(', ')}) CC_GENERIC((#{gen_args.map { |a| "((typeof(#{a.format})){0})" }.join(', ')}), #{macro}, #{t_params.uniq.map { |p| "(#{namespace}_#{p})" }.join(', ')})"
    else
        sym_refs << "#define #{template}_Ref(#{'t'.format}) CC_GENERIC((((typeof(#{'t'.format})){0})), #{macro}, #{t_params.uniq.map { |p| "(#{namespace}_#{p})" }.join(', ')})"
    end
}
